...
void Bolugi_Long()
{
  ...
  NextBuyPrice=RangeMB0+InpStepC*_Point;
   ...
   PL7=NormalizeDouble(PL0-((100.00*InpRangeM)/100*_Point+InpDistTakeProfit*_Point),_Digits);    
   Calculate_V1();
   Calculate_V2(); 
   Calculate_V3(); 
   Calculate_V4(); 
   ...
   if (PositionSelectByTicket(TicketNumber))
{
   ...
   NormalizeDouble(ClosePrice,_Digits);
     ...
     {
     double lastBuyStopLoss=0.0,lastBuyPosition=0.0;
         if(PositionSelectByTicket(TicketNumber))
              {
                  if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
                   {
                lastBuyStopLoss = PositionGetDouble(POSITION_SL);
                lastBuyPosition=PositionGetDouble(POSITION_PRICE_CURRENT);
                     }
                 
              }
      if(lastBuyStopLoss==lastBuyPosition)
               {
               CloseBuyPositions();
               CONTOR=0;
